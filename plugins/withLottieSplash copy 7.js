...
// ✅ نسخة مصححة بالكامل لـ withSplashActivity
...
class SplashActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val minSplashDuration = 17000L // مدة الأنيميشن الخاصة بك

        thread {
            try {
                // 1. هنا نترك التطبيق يقوم بعمليات التحميل في الخلفية
                performAppInitialization()
            } catch (e: Exception) {
                e.printStackTrace()
            }

            // 2. حساب الوقت المتبقي بدقة
            val elapsedTime = System.currentTimeMillis() - startTime
            val remainingTime = maxOf(0, minSplashDuration - elapsedTime)

            if (remainingTime > 0) {
                try { Thread.sleep(remainingTime) } catch (e: InterruptedException) {}
            }

            // 3. الانتقال الفوري والمحسن
            runOnUiThread {
                val intent = Intent(this@SplashActivity, MainActivity::class.java)
                // الأعلام التالية تمنع أي تأخير في تشغيل الـ MainActivity
                intent.addFlags(Intent.FLAG_ACTIVITY_NO_ANIMATION)
                startActivity(intent)
                finish()
                // إلغاء أنيميشن الانتقال الخاص بالنظام لجعل الدخول لحظياً
    ...
    private fun performAppInitialization() {
        // يمكنك وضع أي عمليات تحميل هنا، حالياً هي مجرد محاكاة
